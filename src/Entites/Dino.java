...
public class Dino {
    ...
    private dinoListener listener;

    public Dino(BufferedImage[][] bufferedImage) throws Exception {
        velocity = -22;
        animationSpeed = 12;
        currState = Dino.IDLE;
    ...
    public void draw(Graphics2D g2d, Component c) {
        ...
        g2d.drawImage(bufferedImage[currState][dinoImgIdx], Dino.X_COORDINATE, dinoY, c);
    ...
    public void animate() {
        ...
        if (animationTick >= animationSpeed) {
            ...
            if (dinoImgIdx >= getStateLength(currState) - 1) {
                ...
                if (!isJumpProgress) dinoY = Dino.Y_COORDINATE;
            } else {
                if (currState == Dino.DEATH) dinoY = Dino.Y_COORDINATE;
                dinoImgIdx++;
            }
        }
    }
    ...
    public void setListener(dinoListener listener) {
        this.listener = listener;
    }

    public Point getImageSize() {
        if (imageDimensions.get(currState) == null) try {
            throw new Exception("No Image dimensions found for the state");
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        if (dinoImgIdx >= imageDimensions.get(currState).length)
            return imageDimensions.get(currState)[imageDimensions.get(currState).length - 1];

        return imageDimensions.get(currState)[dinoImgIdx];
    }
